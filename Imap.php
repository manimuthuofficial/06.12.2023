	...
	*/
	...
	public function get_message(int $uid)
	{
		...
		$email = [
			...
			//'original_message_id' => $this->getOriginalMessageId($header),
	...
	public function get_original_message($replyMessageId)
	{
		$searchCriteria = 'HEADER Message-ID "' . $replyMessageId . '"';
		$originalMessageUids = $this->search($searchCriteria);

		if (!empty($originalMessageUids)) 
		{
			$originalMessageUid = reset($originalMessageUids); // Get the first UID

			// Fetch and return details of the original message
			return $this->get_message($originalMessageUid);
		}
	...
	private function getOriginalMessageId($header)
	{
		// Check "In-Reply-To" header first
		if (isset($header->in_reply_to) && !empty($header->in_reply_to)) {
			return $header->in_reply_to;
		}

		// If "In-Reply-To" is not present, check "References" header
		if (isset($header->references) && !empty($header->references)) {
			$references = explode(' ', $header->references);
			return end($references); // Return the last reference as the original message ID
		}

		// If both headers are not present, fallback to the standard "Message-ID"
		return isset($header->message_id) ? $header->message_id : '';
	}
	*/
